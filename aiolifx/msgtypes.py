    ...
    def get_payload(self):
        ...
        for tile_device in self.tile_devices:
            tile_devices += b"".join(
                ...
                little_endian(bitstring.pack("uint:32", tile_device["device_version_vendor"])),
                little_endian(bitstring.pack("uint:32", tile_device["device_version_product"])),
                little_endian(bitstring.pack("uint:64", tile_device["firmware_build"])),
                little_endian(bitstring.pack("uint:16", tile_device["firmware_version_minor"])),
                little_endian(bitstring.pack("uint:16", tile_device["firmware_version_major"])),
            )
        tile_devices_count = little_endian(bitstring.pack("uint:8", self.tile_devices_count))
